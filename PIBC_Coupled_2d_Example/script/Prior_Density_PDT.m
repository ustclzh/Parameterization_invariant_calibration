function y=Prior_Density_PDT(D,V,v)
% v = 1 proposed method 
% v = 0 noninformative
...
if v == -1 % proposed one, non-informative for cholesky
    prior_type = 1;
...
elseif prior_type == 3
    ...
    L2=L-diag(L1);
    n_d=v+1-(1:d);
    y = prod(chi2pdf(L1.^2,n_d)).*prod(normpdf(L2(:)))/prod(diag(L)'.^(d-1:-1:0));
end
end