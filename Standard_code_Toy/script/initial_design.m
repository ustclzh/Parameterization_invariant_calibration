function data=initial_design(data)
    ...
    switch dist
        case 1
            ...
            c=zeros(1000*q_0,d*(d+1)/2);
            n=1;
            for i=1:1000*q_0
                L=chol(invparametrization(b(i,:),dist))';
                if max(abs(log(eig(L*L'))))<log(10)
                    c(n,:)=b(i,:);
                    n=n+1;
                end
            end
        case 2
            a=log(range(2))*(d*(d+1)/2)^(1/2);
            b=randsphere(100*q_0,d*(d+1)/2,a);
            c=zeros(100*q_0,d*(d+1)/2);
            n=1;
            for i=1:100*q_0
                L=logm(invparametrization(b(i,:),dist));
                if max(eig(L))<log(10)
                    c(n,:)=b(i,:);
                    n=n+1;
                end
...
function Y = rand_PDT(n,d,r)
Y=zeros(n,d*(d+1)/2);
for i=1:n
    X=rand(1,d)*(r-0.1)+0.1;
    X=diag(X);
     set=nchoosek(1:d,2);
     for j=1:size(set,1)
         theta=rand(1)*2*pi;
        R=givens_rotation(d,set(j,1),set(j,2),theta);
        X=R'*X*R;
     end
...
function D_para=generate_design_finite_region(sample,d_desire,data)
    ...
    Design_index=Minimax_Design_from_Sample(sample,d_desire,0.6,1,2,2);
